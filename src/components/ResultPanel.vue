      ...
      <div class="result-split">
              ...
              >
                ...
                <td>
                  <div class="stack-cell">
                    ...
                    <span v-for="row in leftoverList(item)" :key="'left-' + row.cargoId" class="left-tag">
                      未装下 {{ row.name }} {{ row.l }}×{{ row.w }}×{{ row.h }} cm ×{{ row.leftover }} 件
                    </span>
                  </div>
                </td>
        ...
        <div v-if="activeResult" class="result-detail">
          ...
          <div v-if="leftoverRows.length" class="detail-block leftover-block">
            <div class="picker-head">
              <span>未装下货物 · {{ leftoverRows.length }} 种 · {{ leftoverCount(activeResult) }} 件</span>
            </div>
            <div class="cargo-table-wrap">
              <table class="cargo-table pack-result-table">
                <thead>
                  <tr>
                    ...
                    <th class="num">高 cm</th>
                    <th>可翻转</th>
                    <th class="num">需求件数</th>
                    <th class="num">已装件数</th>
                    <th class="num">未装下件数</th>
                  </tr>
                </thead>
                <tbody>
              ...
              <table class="cargo-table pack-result-table">
                ...
                <tbody>
                  ...
                  <tr v-for="row in leftoverRows" :key="'left-' + row.cargoId" class="is-left">
                    <td>未装下</td>
                    <td>{{ row.name }} {{ row.l }}×{{ row.w }}×{{ row.h }}</td>
...
function leftoverList(result) {
  ...
  for (const board of result.boards || []) {
    for (const group of board.groups || []) {
      ...
      packedById.set(key, (packedById.get(key) || 0) + group.count);
    }
  }
  const fromCargos = (result.cargos || [])
    .filter((row) => Number(row.l) > 0 && Number(row.w) > 0 && Number(row.h) > 0 && Number(row.qty) > 0)
    .map((row) => {
      const requested = Math.floor(Number(row.qty));
      const loaded = packedById.get(String(row.id)) || 0;
      return {
        cargoId: row.id,
        name: String(row.name || "").trim() || `${row.l}×${row.w}×${row.h}`,
        l: Number(row.l),
        w: Number(row.w),
        h: Number(row.h),
  ...
  if (fromCargos.length) return fromCargos;
  return (result.leftover || []).map((row) => ({
    cargoId: row.cargoId,
    name: String(row.name || "").trim() || `${row.l}×${row.w}×${row.h}`,
    l: Number(row.l) || 0,
    w: Number(row.w) || 0,
    ...
    leftover: Number(row.leftover) || 0,
  }));
}
